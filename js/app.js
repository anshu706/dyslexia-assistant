...
class DyslexiaAssistant {
  ...
  setupEventListeners() {
    ...
    document
      ...
      .addEventListener("click", () => this.toggleTTS());
    document
      .getElementById("ttsStopBtn")
      .addEventListener("click", () => this.stopTTS());
    document.getElementById("ttsSpeed").addEventListener("input", (e) => {
      const value = parseFloat(e.target.value);
      this.tts.setRate(value);
      document.getElementById("ttsSpeedDisplay").textContent =
        value.toFixed(1) + "x";
    });
    ...
    document
      ...
      .addEventListener("click", () => this.handleTextInput());
    ...
    document.getElementById("lineHeight").addEventListener("input", (e) => {
      document.getElementById("lineHeightDisplay").textContent = parseFloat(
        e.target.value,
      ).toFixed(1);
    ...
    document.getElementById("letterSpacing").addEventListener("input", (e) => {
      document.getElementById("letterSpacingDisplay").textContent =
        parseFloat(e.target.value).toFixed(2) + "em";
      this.updateLetterSpacing(e.target.value);
    });

    document
      .getElementById("fontFamily")
      .addEventListener("change", (e) => this.updateFontFamily(e.target.value));
    document
      .getElementById("theme")
      .addEventListener("change", (e) => this.updateTheme(e.target.value));
    document
      .getElementById("contrast")
      .addEventListener("change", (e) => this.updateContrast(e.target.checked));
    document
    ...
    document
      ...
      ?.addEventListener("input", (e) => this.searchInDocument(e.target.value));
    document.addEventListener("mousemove", (e) => this.updateRulerPosition(e));
    document
      .getElementById("textDisplay")
      ?.addEventListener("dblclick", (e) => this.handleWordLookup(e));

    // Custom Modal Event Listeners
    document
      .getElementById("closePasteModal")
      .addEventListener("click", () => this.hidePasteModal());
    document
      .getElementById("cancelPasteBtn")
      .addEventListener("click", () => this.hidePasteModal());
    document
      .getElementById("submitPasteBtn")
      .addEventListener("click", () => this.submitPasteText());
    document
      .getElementById("cancelDeleteBtn")
      .addEventListener("click", () => this.hideConfirmModal());

    // Word Search Popup
    document.getElementById("popupGoogleBtn").addEventListener("click", () => {
      const word = this._popupSelectedText;
      if (word)
        window.open(
      ...
      this.dismissWordPopup();
    });
    document
      .getElementById("popupHighlightBtn")
      .addEventListener("click", () => {
        const word = this._popupSelectedText;
        if (word && this.highlightManager) {
          this.highlightManager.addHighlight(
            word,
            this.highlightManager.currentColor,
          );
    ...
    document
      ...
      .addEventListener("click", () => this.dismissWordPopup());
    // Dismiss when clicking outside the popup
    document.addEventListener("mousedown", (e) => {
      const popup = document.getElementById("wordSearchPopup");
      if (
        popup &&
  ...
  switchPage(pageName) {
    ...
    if (pageIndex[pageName] !== undefined) {
      document
        .querySelectorAll(".nav-tab")
        [pageIndex[pageName]].classList.add("active");
    }
  ...
  loadDocument(docId) {
    ...
    if (doc) {
      ...
      document.getElementById("textDisplay").innerHTML = this.formatContent(
        doc.content,
      );
      ...
      this.storage.updateDocument(docId, {
  ...
  toggleTheme() {
    ...
    document.getElementById("themeToggle").textContent = icon[next];
  }
  ...
  applySettings() {
    ...
    if (this.settings.overlayColor)
      this.setOverlayColor(this.settings.overlayColor);
  }
  ...
  onTTSWordChange(wordData) {
    ...
    document.getElementById("ttsCurrentWord").textContent =
      wordData.word || "...";
  ...
  clearCurrentWordHighlight() {
    ...
    const highlightedElements =
      textDisplay.querySelectorAll(".tts-current-word");
  ...
  setupHighlighting() {
    ...
    if (colorPicker) {
      colorPicker.innerHTML = this.highlightManager.colors
        .map(
          (color) =>
  ...
  handleTextSelection() {
    ...
    this.showWordPopup(selected, rect);
  }

  showWordPopup(text, rect) {
    this._popupSelectedText = text;

    const popup = document.getElementById("wordSearchPopup");
    const label = document.getElementById("wordSearchPopupText");
    const POPUP_H = 90; // approx height
    const MARGIN = 8;

    // Truncate label for display
    const display = text.length > 36 ? text.substring(0, 33) + "…" : text;
    ...
    // Vertical: prefer above selection
    let top = rect.top + window.scrollY - POPUP_H - MARGIN;
    const arrowEl = document.getElementById("wordSearchPopupArrow");
    if (top < window.scrollY + MARGIN) {
      ...
      arrowEl.style.borderBottom = "8px solid var(--bg-secondary, #fff)";
    } else {
      arrowEl.style.top = "auto";
      arrowEl.style.bottom = "-8px";
      arrowEl.style.borderBottom = "none";
      arrowEl.style.borderTop = "8px solid var(--bg-secondary, #fff)";
    }
  ...
  dismissWordPopup() {
    ...
    this._popupSelectedText = null;
    // Don't clear selection here — user may want to keep it
  }
  ...
  displayHighlights() {
    ...
    if (highlights.length === 0) {
      list.innerHTML =
        '<p style="color: var(--text-muted); font-size: 14px;">📂 No highlights yet. Select text to highlight.</p>';
      return;
    }
  ...
  applyHighlightsToText() {
    ...
    highlights.forEach((hl) => {
      ...
      paragraphs.forEach((p) => {
        ...
        p.innerHTML = p.innerHTML.replace(
          regex,
          `<mark style="background-color: ${hl.color}; padding: 2px 4px; border-radius: 2px;">$1</mark>`,
        );
      });
    });
  }
  ...
  displayNotes() {
    ...
    if (notes.length === 0) {
      list.innerHTML =
        '<p style="color: var(--text-muted); font-size: 14px;">📂 No notes yet. Click "Add Note" to create one.</p>';
      return;
    }
  ...
  displayBookmarks() {
    ...
    if (docBookmarks.length === 0) {
      list.innerHTML =
        '<p style="color: var(--text-muted); font-size: 14px;">📂 No bookmarks yet. Click "Bookmark Current" to create one.</p>';
      return;
    }

    list.innerHTML = docBookmarks
      .map(
        (bm) => `
            <div style="
                background: var(--bg-primary);
  ...
  endReadingSession() {
    ...
    this.updateStatistics();
    console.log(
      `📊 Session ended: ${wordsRead} words in ${this.stats.formatTime(timeSpentSeconds)}`,
    );
  }
  ...
  updateStatistics() {
    ...
    document.getElementById("statWords").textContent = this.stats.formatWords(
      totalStats.totalWordsRead,
    );
    document.getElementById("statTime").textContent = this.stats.formatTime(
      totalStats.totalTimeSpent,
    );
    document.getElementById("statStreak").textContent =
      totalStats.readingStreak;
  ...
  displayRecentSessions() {
    ...
    if (recentSessions.length === 0) {
      container.innerHTML =
        '<p style="color: var(--text-muted); text-align: center;">No reading sessions yet. Start reading to see your stats!</p>';
      return;
    }
  ...
  toggleBionicReading(forcedState) {
    this.bionicEnabled =
      forcedState !== undefined ? forcedState : !this.bionicEnabled;
    const textDisplay = document.getElementById("textDisplay");
    ...
    if (this.bionicEnabled) {
      const formatted = this.currentDoc.content
        ...
        .map(
          (p) =>
            `<p class="bionic-reading">${Utils.applyBionicReading(this.escapeHtml(p.trim()))}</p>`,
        )
        .join("");
  ...
  toggleReadingRuler(forcedState) {
    this.rulerEnabled =
      forcedState !== undefined ? forcedState : !this.rulerEnabled;
    ...
    if (rulerBtn) {
      rulerBtn.style.borderColor = this.rulerEnabled
        ? "var(--accent)"
        : "var(--border)";
    }
    if (rulerCheckbox) {
  ...
  async handleWordLookup(e) {
    ...
    body.innerHTML =
      '<div style="padding: 20px; text-align: center; color: var(--text-muted);">🔍 Fetching definition...</div>';
    modal.style.display = "flex";
